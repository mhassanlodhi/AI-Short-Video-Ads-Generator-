import { DollarSignIcon, FolderEditIcon, GalleryHorizontalEnd, Link, MenuIcon, SparkleIcon, XIcon } from 'lucide-react';
        ...
        >
            <div className='max-w-6xl mx-auto flex items-center justify-between bg-black/50 backdrop-blur-md border border-white/4 rounded-2xl p-3'>
                ...
                <div className='hidden md:flex items-center gap-8 text-sm font-medium text-gray-300'>
                    {navLinks.map((link) => (
                        <NavLink onClick={() => scrollTo(0, 0)} to={link.href} key={link.name} className="hover:text-white transition">
                            {link.name}
                        </NavLink>
                    ))}
                </div>
