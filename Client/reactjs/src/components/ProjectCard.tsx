...
import { EllipsisIcon, Ghost, ImageIcon, Loader2Icon, PlaySquareIcon, Share2Icon, Trash2Icon } from "lucide-react";
...
const ProjectCard = ({gen, setGenerations, forCommunity = false} :
  ...
  return (
    ...
    overflow-hidden hover:border-white/20 transition group">
        {/* Preview */}
        ...
        'aspect-video'} relative overflow-hidden`}>
            ...
            {(!gen?.generatedImage && !gen?.generatedVideo) && (
                ...
                items-center justify-center bg-blaack/20">
                    <Loader2Icon className="size-7 animated-spin"/>
                </div>
            )}
            ...
            <div className="absolute left-3 top-3 flex gap-2 items-center">
             ...
             {!forCommunity && (
                ...
                group-hover:opacity-100 transition flex items-center gap-2">
                    ...
                    <div className="flex flex-col items-end w-32 tex-sm">
                        ...
                        rounded-lg shadow-md mt-2 py-1 z-10`}>
                            ...
                            hover:bg-black/10 cursor-pointer"> <ImageIcon size={14} />
                            Download Image</a>}

                            {gen.generatedVideo && <a href="#" download
                            className="flex gap-2 items-center px-4 py-2
                            hover:bg-black/10 cursor-pointer"> <PlaySquareIcon size={14} />
                            Download Video</a>}

                            {(gen.generatedVideo || gen.generatedImage ) && <button
                                onClick={()=> navigator.share({url : gen.generatedVideo
                                    || gen.generatedImage, title: gen.productName, text: gen.productDescription})}
            ...
            <div className="flex items-start justif-between gap-4">
                ...
                <div className="text-right">
                    <div className="mt-2 flex flex-col items-end gap-1">
                        ...
                        rounded-full">Aspect: {gen.aspectRatio}</span>
                    </div>
                </div>
            </div>
            {/*Product Description*/}
