import { Loader2, Loader2Icon, RectangleHorizontalIcon, RectangleVerticalIcon, Upload, Wand2Icon } from "lucide-react" 
...
const Generator = () => {
  ...
  const [userPrompt, setrUserPrompt] = useState('')
    ...
    <div className="min-h-screen text-white p-6 md:p-12 mt-28">
        ...
        <form onSubmit={handleGenerate} className="max-w-4xl mx-auto mb-40">
             ...
             justify-between">
                ...
                <div className="w-full">
                    <div className="mb-4 text-gray-300">   
                        ...
                        <input type="text" id="name" value={name} onChange={(e)=>setName(e.target.value)} placeholder="Name your Project" 
                        required className="w-full bg-white/3 rounded-lg border-2 p-4 text-sm
                        border-violet-200/10 focus:border-vioet-500/50 outline-none transitioned all"/>
                    </div>
                    <div className="mb-4 text-gray-300">   
                        <label htmlFor="productName" className="block text-sm mb-4">Product Name</label>
                        <input type="text" id="productName" value={productName} onChange={(e)=>setProductName(e.target.value)} placeholder="Enter the name of the Product" 
                        required className="w-full bg-white/3 rounded-lg border-2 p-4 text-sm
                        border-violet-200/10 focus:border-vioet-500/50 outline-none transitioned all"/>
                    </div>
                    <div className="mb-4 text-gray-300">   
                        ...
                        <textarea id="productDescription" rows={4} value={productDescription}
                         ...
                         placeholder="Enter the description of the Product" className="w-full 
                         bg-white/3 rounded-lg border-2 p-4 text-sm border-violet-200/10
                         focus:border-violet-500/50 outline-none resize-none transition-all"/>
                    </div>
                    <div className="mb-4 text-gray-300"> 
                        <label className="block text-sm mb-4">Aspect Ratio</label>
                        <div className="flex gap-3">
                            <RectangleVerticalIcon onClick={()=>setAspectRatio('9:16')}
                                className={`p-2.5 size-13 bg-white/6 rounded transitioned-all ring-2
                    ...
                    <div className="mb-4 text-gray-300">   
                        ...
                        <textarea id="userPrompt" rows={4} value={userPrompt}
                         onChange={(e)=>setrUserPrompt(e.target.value)} 
                         placeholder="Describe how you want the narration to be." className="w-full 
                         bg-white/3 rounded-lg border-2 p-4 text-sm border-violet-200/10
                         focus:border-violet-500/50 outline-none resize-none transition-all"/>
                    </div>
                </div>
            ...
            <div className="flex justify-center mt-10">
                ...
                disabled:opacity-70 disabled:cursor-not-allowed" type="submit">
                    {isGenerating ? (
                        <>
